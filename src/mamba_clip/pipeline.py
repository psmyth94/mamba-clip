...
def ray_tune_pipeline(args):
        ...
        def setup(self, args):
            ...
            if args.stage == 1:
                ...
                if tokenizer is not None:
                    ...
                    if args.class_weighted_loss is not None:
                        loss = partial(
                            cross_entropy_loss, weight=args.class_weighted_loss
                        )
                    else:
                        loss = cross_entropy_loss
            elif args.stage == 2:
                ...
                model_stage_2 = ClipClassifier(
                    ...
                    feature_dim=1024
                    if not (
                        args.use_visual_only
                        or args.use_text_only
                        or args.use_inner_prod
                    )
                    else 512,
                    num_classes=args.num_classes,
                    use_visual_only=args.use_visual_only,
    ...
    tuner = tune.Tuner(
        tune.with_resources(
            Trainable, {"gpu": 1 if torch.cuda.is_available() else 0, "cpu": 4}
        ),
